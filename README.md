...
- **🔧 系统稳定性提升**
  ...
  - 优化日志系统在 Next.js 15 + Turbopack 环境下的兼容性

#### 📦 核心功能增强

##### 👥 主/子 Key 架构体系 ⭐

完整的层级权限管理系统，支持精细化的 API 密钥管理：

- **主 Key（汇总视角）**
  - 可以新建和管理多个子 Key
  - 查看所有子 Key 的使用数据（汇总统计）
  - 控制子 Key 的权限和配额
  - 独立的 API 密钥管理页面
...
- **供应商分组**
  - 创建多个供应商分组（如 `super code`、`个人测试`、`墨颜 claude` 等）
  - 分组标签显示供应商数量
  - 支持自定义分组标签
  - 调度规则按分组隔离
